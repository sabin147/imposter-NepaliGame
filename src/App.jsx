...
import { Ghost, Search, RotateCcw, Sparkles, PartyPopper, ChevronRight } from 'lucide-react';
              ...
              <div className="space-y-6">
                <div className="bg-slate-50 p-4 rounded-3xl border-2 border-slate-100">
                  ...
                  <div className="flex items-center justify-around">
                    ...
                    <span className="text-4xl font-black text-purple-600">{playerCount}</span>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 h-48 overflow-y-auto pr-1 custom-scrollbar">
                    <button onClick={() => setSelectedCategory('all')} className={`p-3 rounded-2xl text-xs transition-all border-2 ${selectedCategory === 'all' ? 'bg-purple-600 text-white border-purple-600 shadow-md scale-95' : 'bg-white text-slate-600 border-slate-100'}`}>🌈 All Random</button>
                    {Object.entries(categories).map(([key, cat]) => (
                      <button key={key} onClick={() => setSelectedCategory(key)} className={`p-3 rounded-2xl text-xs transition-all border-2 ${selectedCategory === key ? 'bg-purple-600 text-white border-purple-600 shadow-md scale-95' : 'bg-white text-slate-600 border-slate-100'}`}>{cat.name}</button>
                    ))}
                  </div>
                </div>
              </div>

              <button onClick={startGame} className="pop-button w-full bg-gradient-to-r from-purple-600 to-indigo-600 p-5 rounded-3xl text-white text-xl font-black shadow-xl hover:brightness-110 active:scale-95 transition-all">START GAME! 🚀</button>
            </div>
          )}
          ...
          {gameState === 'playing' && (
            <div className="text-center space-y-8 animate-in slide-in-from-right duration-300">
              <div className="space-y-2">
                <div className="inline-block px-6 py-2 bg-purple-100 text-purple-600 rounded-full text-sm font-black">Player {currentPlayer + 1} of {playerCount}</div>
                <p className="text-slate-400 text-xs uppercase tracking-widest font-bold">Category: {gameData.categoryName}</p>
              </div>

              {!showWord ? (
                <div className="py-4">
                  <button onClick={() => setShowWord(true)} className="w-48 h-64 bg-gradient-to-br from-pink-400 to-purple-500 rounded-[2.5rem] mx-auto flex flex-col items-center justify-center shadow-2xl pop-button border-8 border-white group">
                    <Search size={64} className="text-white group-hover:scale-125 transition-transform" />
                ...
                <div className="space-y-8 animate-in flip-in-y duration-500">
                  <div className={`p-10 rounded-[2.5rem] border-4 border-dashed relative ${currentPlayer === gameData.imposterIndex ? 'bg-red-50 border-red-400' : 'bg-yellow-50 border-yellow-400'}`}>
                     ...
                     <div className={`text-4xl font-black tracking-tighter ${currentPlayer === gameData.imposterIndex ? 'text-red-600' : 'text-purple-800'}`}>
                       {currentPlayer === gameData.imposterIndex ? 'तपाईँ चोर! 🕵️\n(IMPOSTER)' : gameData.secretWord}
                     </div>
                  </div>
                  <button onClick={nextPlayer} className="w-full bg-slate-800 text-white p-5 rounded-3xl text-xl font-black pop-button flex items-center justify-center gap-2">I UNDERSTAND <ChevronRight size={24} /></button>
                </div>
              )}
            </div>
          )}
            ...
            <div className="text-center space-y-8 animate-in zoom-in duration-500">
              ...
              <div className="space-y-3">
                ...
                <div className="p-6 bg-red-50 rounded-3xl border-2 border-red-100 text-left">
                  <p className="text-xs text-red-600 uppercase mb-1 font-bold">The Imposter Was:</p>
                  <p className="text-3xl font-black text-red-700">Player {gameData.imposterIndex + 1}</p>
                </div>
              </div>

              <button onClick={() => setGameState('setup')} className="pop-button w-full bg-gradient-to-r from-yellow-400 to-orange-500 p-5 rounded-3xl text-white text-xl font-black flex items-center justify-center gap-2">PLAY AGAIN <RotateCcw size={24} /></button>
            </div>
          )}
        </div>
      </div>
